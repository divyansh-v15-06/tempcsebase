...
const PublicTopNavbar = () => {
        ...
        const fetchFacultyDetails = async () => {
            try {
                ...
                if (data.success && data?.data?.name) {
                    ...
                    sessionStorage.setItem('facultyName', data.data.name)
                    // we are getting numeric id from backend
                    sessionStorage.setItem('facultyId', facultyId as string)
                } else {
    ...
    const fetchData = async () => {
        ...
        const url = `${process.env.NEXT_PUBLIC_API_URL}/facultyInfo/get/${facultyId}`
        try {
            ...
            <div className='container mx-auto flex items-center justify-between'>
                ...
                <div className='flex items-center space-x-4'>
                    <div className='w-[4rem]'>
                        <img src={photo} alt={name} className='rounded-full h-16 w-16' />
                    </div>
                    <div>
                ...
                <div className='flex gap-3 items-center'>
                    <Link href={facultyInfo.googleScholar||""} target='_blank'>
                        <svg
                    ...
                    <Link href='/faculty/login' className='ml-5 px-5 fill-current cursor-pointer bg-white p-2 rounded-lg'>
                    Login 
                    </Link>
