...
import AdminModalPersonalinfo from '@/components/admin-components/Modals/adminModalPersonalinfo'
import Link from 'next/link'
import AdminModalPersonalInfoUpdate from '@/components/admin-components/Modals/adminModalPersonalInfoUpdate'
...
export default function ProjectsAdmin({ }: Props) {
    ...
    const fetchAdminExp = () => {
        const userId = sessionStorage.getItem('userId')
        const urlWithParams = `${process.env.NEXT_PUBLIC_API_URL
            }/AdministrativeExperience/get?facultyId=${userId}`
        ...
        axios
            ...
            .then((response) => {
                ...
                setFacultyInfo((prev) => ({
                    ...prev,
                    administrativeExperience: calculateNetYearsAndMonths2(response.data.data),
                }));
            })
            .catch((error) => {
                console.error('Error fetching data:', error)
    ...
    function calculateNetYearsAndMonths2(intervals) {
        console.log(intervals, 'intervals');

        let totalMonths = 0;

        for (const interval of intervals) {
            const fromDate = new Date(interval.startDate);
            const toDate = interval.endDate === "Present" ? new Date() : new Date(interval.endDate);
    ...
    const handleModalSubmit = (formData) => {
        axios
            .post(`${process.env.NEXT_PUBLIC_API_URL}/facultyInfo`, formData, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('access_token') as string}`,
                    'Content-Type': 'application/json',
                },
            ...
            .catch((error) => {
                toast.error('Error adding Personal Info')
                console.error(error)
            })
    }
    ...
    return (
            ...
            <div className='p-4 h-full  bg-white shadow-md rounded-md'>
                ...
                <div className='flex justify-end items-center  p-4'>
                    <div className='flex justify-center gap-8 '>
                        <Button
                            className={`bg-blue-600 hover:bg-blue-500 transition-all duration-150 ease-out text-white border-3 border rounded-3xl px-4 py-2 ${isDataPresent ? 'block' : 'hidden'}`}
                ...
                {isDataPresent?<div className='flex flex-col  mt-5 justify-between items-center mb-4'>
        ...
        <p className="text-gray-500">No personal information available. Please add your details.</p>
      </div>}
            </div>


            <AdminModalPersonalinfo
                isOpen={isModalOpen}
                onClose={() => setIsModalOpen(false)}
                onSubmit={handleModalSubmit}
            />
            <AdminModalPersonalInfoUpdate
