...
export default function PatentsAdmin() {
    ...
    const fetchData = () => {
        ...
        }/patent/get/admin?startYear=${filter1.trim()}&name=${filter2.trim()}&status=${filter3.trim()}&endYear=${filter5.trim()}`
        axios
            .get(urlWithParams)
            .then((response) => {
    ...
    const patents = data.data.find((item) => item.id === id)
    if (patents) {
        setinitialdata(patents)
...
const handleUpdateSubmit=(formData,id)=>{
    axios
            .put(`${process.env.NEXT_PUBLIC_API_URL}/patent/update/${id}`, formData, {
                headers,
            })
            .then((response) => {
    ...
    return (
        <div className='h-[92vh]'>
            ...
            <div className='flex justify-between items-center border-b-2 p-4'>
                <h1 className='font-semibold text-2xl'>Patents Page</h1>
...
function Table({ patents, isLoading, onUpdate, onDelete }) {
    return (
