...
export default function ProjectsAdmin({ }: Props) {
    ...
    const fetchData = () => {
        ...
        const urlWithParams = `${process.env.NEXT_PUBLIC_API_URL
            }/project/get/admin?startYear=${filter1.trim()}&fundingAgency=${filter3.trim()}&endYear=${filter5.trim()}`
    ...
    const handleUpdate = (id) => {
        const patents = data.find((item) => item.id === id)
        if (patents) {
    ...
    return (
        <div className='h-[92vh]'>
            ...
            <div className='flex justify-between items-center border-b-2 p-4'>
                <h1 className='font-semibold text-2xl'>Projects Page</h1>
